...
Each JSONL line includes `authors` (array), `source` (array of strings such as `notion:MMDD-n` from the Document ID bullet, `slack:<channel>`, or `gitlab:<project>`), and summaries (`summary_en` plus the localized summary key for the configured language).

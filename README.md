...
Each JSONL line includes `authors` (array), `source` (prefixed as `notion:<Document ID>`, `slack:<channel>`, or `gitlab:<project>`), and summaries (`summary_en` plus the localized summary key for the configured language).
